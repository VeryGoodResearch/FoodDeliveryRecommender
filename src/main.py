...
def parse_global_args(parser):
    ...
    parser.add_argument('--train_test_final',type=int,default=0)
    return parser
...
def main():
    ...
    if args.train > 0:
        ...
        logging.info('Dataset user features: '+','.join(data_dict['test'].corpus.user_feature_names))
        logging.info('Dataset item features: '+','.join(data_dict['test'].corpus.item_feature_names))
        logging.info('Dataset context features: '+','.join(data_dict['test'].corpus.context_feature_names))
    if args.train_test_final:
        eval_res = runner.print_res(data_dict['train'])
        logging.info(os.linesep + 'Train  After Training: ' + eval_res)
...
if __name__ == '__main__':
    ...
    init_parser.add_argument('--model_name', type=str, default='BPRMF_test', help='Choose a model to run.')
    ...
    args.data_appendix = ''
    if model_name.reader in  ['ContextReader','ContextSeqReader','ContextSeqCReader']:
        args.data_appendix = '_context%d%d%d'%(args.include_item_features,args.include_user_features,
                                        args.include_context_features)
    if model_name.reader in ['ContextScoreReader']:
        args.data_appendix = '_%s_%s'%(args.base_model1,args.base_model2)

    # Logging configuration
    log_args = [init_args.model_name, args.dataset+args.data_appendix, str(args.random_seed),
                str(args.test_repeat), str(args.test_exploration)]
