...
def main():
    ...
    if os.path.isfile(os.path.join(args.log_dir, args.out_f)):
        ...
        if (df.columns.to_numpy() != np.array(columns)).sum():
            overwrite = input('Warning! There exists a file called %s with different format, overwrite it? y/n (default n)'%(args.out_f))
    ...
    for cmd in lines:
        ...
        if args.n > 1:
            ...
            info['Test'] = ','.join(avgs)
            epoch_avgs = np.mean([int(x) if len(x) else 0 for x in df['Best Iter'].tolist()[-args.n:]])
